...
def get_relative_position(absolute_pose, absolute_position):
    ...
    T = np.array([[np.cos(theta),-np.sin(theta),x], [np.sin(theta),np.cos(theta),y]], dtype=np.float32)
    apos = np.array([absolute_position[X], absolute_position[Y], 1], dtype=np.float32)
    relative_position = np.matmul(T,apos)
