...
def sample_random_position(occupancy_grid):
  ...
  # Solution:
  position = np.random.rand(2)*4-np.array([2,2])
...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  for i in range(n+1):
    # check arc existence
    final_pose[2] += np.pi*2/n
    final_node = Node(final_pose)
    center, radius = find_circle(node, final_node)
    r1 = node.position-center
    r2 = final_position-center
    lencheck1 = np.abs(np.linalg.norm(r1)-radius) < e
    lencheck2 = np.abs(np.linalg.norm(r2)-radius) < e
    percheck1 = np.abs(np.dot(node.direction,r1)) < e
    percheck2 = np.abs(np.dot(final_node.direction,r2)) < e
    dircheck = np.dot(node.direction,perpendicular(r1)) * np.dot(final_node.direction,perpendicular(r2)) > 0
    if lencheck1 and lencheck2 and percheck1 and percheck2 and dircheck:
      # check collision
      t1 = np.arctan2(r1[1],r1[0])
      t2 = np.arctan2(r2[1],r2[0])
      dt = t2 - t1
      t = t1
      ...
      if isCollisionFree:
        dcost = np.linalg.norm(GOAL_POSITION-final_position) - np.linalg.norm(GOAL_POSITION-node.position)
        return final_node, dcost
