...
# Use a counter to register the length of time not registered
dt_counter = 0

def euler(current_pose, t, dt):
  
  # Reset counter at 1Hz rate
  global dt_counter
  if dt_counter >= 1:
    dt_counter -= 1
  
  # 1Hz perception-action loop
  t = np.floor(t)
  
  next_pose = current_pose.copy()
  u = 0.25
  w = np.cos(t + dt_counter)
...
def rk4(current_pose, t, dt):
  ...
  k1_th = np.cos(t + dt_counter)
  
  k2_x = u * np.cos(current_pose[YAW] + k1_th*dt/2)
  k2_y = u * np.sin(current_pose[YAW] + k1_th*dt/2)
  k2_th = np.cos(t + dt_counter + dt/2)
  
  k3_x = u * np.cos(current_pose[YAW] + k2_th*dt/2)
  k3_y = u * np.sin(current_pose[YAW] + k2_th*dt/2)
  k3_th = np.cos(t + dt_counter + dt/2)
  
  k4_x = u * np.cos(current_pose[YAW] + k3_th*dt)
  k4_y = u * np.sin(current_pose[YAW] + k3_th*dt)
  k4_th = np.cos(t + dt_counter + dt)
  
  next_pose[X] += 1/6 * (k1_x + 2*k2_x + 2*k3_x + k4_x) * dt
  next_pose[Y] += 1/6 * (k1_y + 2*k2_y + 2*k3_y + k4_y) * dt
  next_pose[YAW] += 1/6 * (k1_th + 2*k2_th + 2*k3_th + k4_th) * dt
