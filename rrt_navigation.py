...
def feedback_linearized(pose, velocity, epsilon):
    ...
    # Solution:
    u = float(velocity[0] * np.cos(pose[YAW]) + velocity[1] * np.sin(pose[YAW]))
    w = float((-velocity[0] * np.sin(pose[YAW]) + velocity[1] * np.cos(pose[YAW])) / epsilon)
...
def get_velocity(position, path_points):
    ...
    # Solution:
    closest_index = 0
    closest_distance = np.linalg.norm(path_points[closest_index]-position)

    for i in range(len(path_points)):
        distance = np.linalg.norm(path_points[i]-position)
        if distance < closest_distance:
            closest_index = i
            closest_distance = distance

    if closest_distance > .2:
        u = path_points[closest_index] - position
    else:
        u = path_points[closest_index+1] - position
    
    v = u/np.linalg.norm(u)*SPEED*0.5
    return v
