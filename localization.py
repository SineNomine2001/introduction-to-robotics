...
def braitenberg(front, front_left, front_right, left, right):
    ...
    # Solution:
    safe_front = front-safety_dist
    safe_front_left = front_left-safety_dist
    safe_front_right = front_right-safety_dist
    safe_left = left-safety_dist
    safe_right = right-safety_dist
    
    u = max_u * (safe_front)/safety_dist
    
    w = max_w * ((safe_front_left-safe_front_right) + 0.5*(safe_left-safe_right))/safety_dist
    
    u = np.clip(u, 0.01, max_u)
...
class Particle(object):
    ...
    def __init__(self, like_pose=None):
        ...
        # Solution:
        self.set_pose(like_pose)
        while not self.is_valid():
            self.set_pose(like_pose)
    
    def set_pose(self, like_pose=None):
        if like_pose is not None:
            self._pose = np.random.rand(3)*0.2-np.array([0.1,0.1,0.1])+like_pose
        else:
            self._pose = np.random.rand(3)*4-np.array([2,2,2])
    ...
    def is_valid(self):
        ...
        # Solution:
        is_in_walls = self._pose[0] > -2 and self.pose[0] < 2 and self._pose[1] > -2 and self.pose[1] < 2
        is_out_cyl = np.linalg.norm(self._pose-np.array([0.5,0.6,0])) > 0.3
        return is_in_walls and is_out_cyl
    ...
    def move(self, delta_pose):
        ...
        # Solution:
        delta_pose *= np.array([1.1,1.1,1.1])-np.random.rand(3)*0.2
        self._pose[0] += delta_pose[0] * np.cos(self._pose[2])
        self._pose[1] += delta_pose[0] * np.sin(self._pose[2])
    ...
    def compute_weight(self, front, front_left, front_right, left, right):
        ...
        # Solution:
        # Assign weights using p(z_t|x_t), where:
        # z_t is the measurements taken from the pose of the particle at time t
        # x_t is the pose of the particle at time t
        if self.is_valid():
            pf = scipy.stats.norm(front, sigma).pdf(self.ray_trace(0.))
            pfl = scipy.stats.norm(front_left, sigma).pdf(self.ray_trace(np.pi/4.))
            pfr = scipy.stats.norm(front_right, sigma).pdf(self.ray_trace(-np.pi/4.))
            pl = scipy.stats.norm(left, sigma).pdf(self.ray_trace(np.pi/2.))
            pr = scipy.stats.norm(right, sigma).pdf(self.ray_trace(-np.pi/2.))
            self._weight = pf+pfl+pfr+pl+pr
        else:
            self._weight = 0.
...
class Localization(Node):
    ...
    def timer_callback(self):
        if self._laser.ready and self._motion.ready and self._groundtruth.ready:
            ...
            # Solution:
            for i, p in enumerate(self._particles):
                if p.weight < mean_weight:
                    new_particles.append(Particle(max_weight_p.pose))
                else:
